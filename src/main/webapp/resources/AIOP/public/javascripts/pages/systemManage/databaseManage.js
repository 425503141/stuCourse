...
var hostManage = (function (model){ 
	    ...
	    /**
		  ...
		  */
	   model.loadDatabaseTypesList = function(obj,buttonWidth){
		   $.AIGet({
		   	   url:$.ctx + "/api/sys/databaseManagement/selectAll",
			   data:{},
			   success:function(result) {
				   		var data = result.data;
				   	    var menuStr = '<option value="">请选择</option>';
				   		for(var i in data){
				   		    menuStr+='<option value="'+data[i].databaseId+'">'+data[i].databaseName +'</option>'	
				   		}
				   		$(obj).html(menuStr);
				   		$(obj).multiselect({
				   			nonSelectedText:"请选择",
        ...
        model.loadHostManageGrid = function(option) {
			...
			$("#gridHostManage").AIGrid({        
				url:$.ctx + '/api/sys/databaseManagement/list',
				datatype: "json",
				colNames:["id",'分类', '系统', '用途','主机名称','主机IP','数据库名称','数据库类型','数据库版本','数据库IP','启动时间','连通状态','内存大小(G)','CPU数','更新时间','操作'],
			   	colModel:[
			   	    {name:'dbId',index:"",hidden:true,frozen : true},
			   		{name:'classify',index:'classify',width:0.06*gridWidth,frozen : true},
			   		{name:'system',index:'system',width:0.06*gridWidth,frozen:true,align:"center"},
			   		{name:'purpose',index:'purpose',width:0.06*gridWidth,frozen:true,align:"center"},
			   		{name:'serverName',index:'server_name',width:0.07*gridWidth,frozen:true,align:"center"},
			   		{name:'serverIpAddr',index:'server_ip_addr', width:0.10*gridWidth,frozen:true,align:"center"},
			   		{name:'dbName',index:'db_name',width:0.08*gridWidth,frozen:true,align:"center"},
			   		{name:'databaseName',index:'database_name',width:0.06*gridWidth,align:"center"},
			   		{name:'dbVersion',index:'db_version',width:0.06*gridWidth, align:"center",formatter:$.setNull},
			   		{name:'dbIpAddr',index:'db_ip_addr ',width:0.10*gridWidth, align:"center"},
			   		{name:'startTime',index:'start_time ',width:0.10*gridWidth,align:"center",formatter:DateFmt.dataDateFormateMinute},
			   		{name:'dbStatus',index:'db_status', width:0.06*gridWidth, align:"center",formatter:$.setIsOn},
			   		{name:'memorySize',index:'memory_size', width:0.06*gridWidth, align:"center",formatter:$.setNull},		
			   		{name:'cpuNum',index:'cpu_num',width:0.06*gridWidth,align:"center",formatter:$.setNull},
			   		{name:'updateTime',index:'update_time',width:0.10*gridWidth,align:"center",formatter:DateFmt.dataDateFormateMinute},	
			...
			function del(cellvalue, options, rowObject){
				var html='<button type="button" class="btn btn-default  ui-table-btn ui-table-btn" data-id="'+rowObject.dbId+'" data-name="'+rowObject.planName+'" onclick="hostManage.deleteHostManageSingle(this);" title="删除">删除</button><button type="button" class="btn btn-default ui-table-btn" data-id="'+rowObject.dbId+'" data-planName="'+rowObject.planName+'" onclick="hostManage.insertOrUpdateApp(this,\'1\')" title="修改">修改</button>';
				return html;
			}
        };
        /**
	    ...
	    */
        model.deleteHostManageSingle = function(obj){
        	...
        	var msg = '确认是否删除该数据库?';
        	var url = $.ctx + "/api/sys/databaseManagement/delete";
			var ajaxData = {"dbId":dataId};
...
//					  });
	    ...
	    */
        model.getHostManageById = function(id) {
	    	$.AIGet({
    			url:$.ctx + "/api/sys/databaseManagement/selectById",
    			datatype:"json",
    			data:{"dbId":id},
    			success:function(result){
    				...
    				var data = result.data;
    				$('#nodeInput').val(data.platformNodeName).attr('purpose',data.nodeId);
    				$('input[name=dbName]').val(data.dbName );
    				$('input[name=serverIpAddr]').val(data.serverIpAddr);
    				$('input[name=dbIpAddr]').val(data.dbIpAddr);
    				$('input[name=port]').val(data.port);
    				$('input[name=userName]').val(data.userName);
    				$('input[name=passWord]').val(data.passWord);
    				//数据库类型
    				$('#dbType ').val(data.dbType).multiselect('refresh');
   			  }
	    	});
        };
	    /**
		...
		model.dataBaseLinkTest = function() {
			  var dbId  = $('#hostManageAddDlg').attr('dbId');
			  	  dbId = dbId ? dbId : "";
			  var purpose = $('#nodeInput').attr('purpose');//所属节点id
			   ...
			   if(!checkObj[1]){
			    ...
			    if(checkObj[1]){
					  $.AIPost( {
					   	   url:$.ctx + "/api/sys/databaseManagement/testMysql",
					   	   data:saveObj,
					   	   success:function(result) {
	    ...
	    model.insertOrUpdateApp = function(obj,isUpdate){
	    	$("#hostManageAddDlg").aiDialog({
	    		  ...
	    		  gridObj:model.gridObj,
	    		  callback:function(){
	    			  var dbId  = isUpdate =='1' ? $(obj).attr('data-id') : "";
	    			  var purpose = $('#nodeInput').attr('purpose');//所属节点id
	    			  var saveObj = formFmt.formToObj($("#formSaveHostManage"));
	    			  saveObj.dbId = dbId;
	    			  saveObj.purpose = purpose;
	    			  ...
	    			  if($('#addMore').hasClass("active")){//批量添加
		    			  ...
		    			  if(checkObj[1]){
		    				  if(isUpdate=="0"){
		    				  	$.AIPost(hostManage.returnAjaxOption($.ctx + "/api/sys/databaseManagement/insert",saveObj));
		    				  	//用来服务器记录log
//		    		    		$.AILog({
//		    		    			  "action": "新增",//动作
//		    		    			  "detail": "",//详情,默认为空
...
//		    		    	    });
		    				  ...
		    				  if(isUpdate=='1'){
		    					  $.AIPut(hostManage.returnAjaxOption($.ctx + "/api/sys/databaseManagement/update",saveObj));
		    					  //用来服务器记录log
...
//			    		    	  });
	    		  ...
	    		  open:function(){
	    			  var dbId  = isUpdate =='1' ? $(obj).attr('data-id') : "";
	    			  $('#hostManageAddDlg').attr('dbId',dbId);
	    			  $("#J_tabs").removeClass("hidden");
					  $("#addMore,.J_addMore").removeClass("active");
					  $("#addOnce,.J_addOnce").addClass("active");
					  $('input[name=serverIpAddr],input[name=dbIpAddr]').removeAttr('disabled');
					  // 清除文件
					  ...
					  file.remove(); 
					  //所在主机输入框模糊下拉搜索
					  $('#nodeInput').attr('disabled','disabled');
					  $("#serverIpAddr").autocomplete({
				    		source:function( request, response ) {
				    	        $.AIGet( {
				    	      	   url:$.ctx + "/api/sys/databaseManagement/getServerIpAddr",
				    	      	   data:{"serverIpAddr":$.trim($("#serverIpAddr").val())},
				    	      	   success:function( data) {
				    	      	  			response($.map(data.data, function(item ) { // 此处是将返回数据转换为 JSON对象，并给每个下拉项补充对应参数
				    	                    	return {
				    	                    		// 设置item信息
				    	                    		value: item["ipAddr"], // 下拉项显示内容
				    	                    	    info: item//返回的每条数据信息
				    	                    	}
				    	      	  			}));
				    	      	  		}
				    	    ...
				    	    select: function( event, ui ) {
				    	    	...
				    	    	$('#nodeInput').val(info.platformNodeName).attr('purpose',info.nodeId);
				    	    }
				    	});
	    			  //操作系统类型下拉
	    			  model.loadDatabaseTypesList('#dbType ','291');
	    			 if(isUpdate&&isUpdate=='1'){
	    				  $("#J_tabs").addClass("hidden");
						  $("#addMore").removeClass("active");
						  $("#addOnce").addClass("active");
						  $('input[name=serverIpAddr],input[name=dbIpAddr]').attr('disabled','disabled');
	    				  var dataId = $(obj).attr('data-id');
	    ...
	    model.addAppManageBatch = function(){
	      	var ajaxData ={"databaseList" : model.databaseList};
	      	$.AIPost({
	      		url: $.ctx + "/api/sys/databaseManagement/insertList",
	      		data: ajaxData,
	      		async:true,
	      		dataType:"json",
	      		type:"POST",
