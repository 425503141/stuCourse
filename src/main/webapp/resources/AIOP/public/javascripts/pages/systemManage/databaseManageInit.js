...
$(function(){
	...
	$('#menuList li.J_nav_database').addClass('active');
	...
	$('#hostManageDeleteBatch').click(function(){
		var ids = $("#gridHostManage").jqGrid('getSelectedIdsArray',{key:"dbId"});
			ids = ids.join(',');
    	var msg = '确认是否删除选中数据库?';
    	var url = $.ctx + "/api/sys/databaseManagement/deleteList";
		var ajaxData = {"dbIds":ids};
		if(!ids){
			$("#appManageDeleteDlg").alert({
				   title:"提示",
				   content:"请选择要删除的数据库",
...
//	    });
	...
	$( "#searchTxt" ).AIAutoComplete({
		url:$.ctx + "/api/sys/databaseManagement/listByNameOrIp",
	  	 data:{
	...
	$(".J_upBox").on("change","#fileExcel",function(){
	  	$.fileUpload({
	  		url:$.ctx + "/api/sys/databaseManagement/databaseManagementListUpload",
	  		...
	  		success:function(data){
	  			...
	  			$(".J_falseDlg").hide();
				if(data.status == 200){
					hostManage.databaseList = data.data;
